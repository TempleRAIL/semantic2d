...
class MapConverter:
    ...
    def lidar2map(self, lidar_pos, lidar_scan):
        ...
        lidar_points = np.zeros((POINTS, 2))
        angles = np.linspace(-(135*np.pi/180), 135*np.pi/180, num=POINTS)
        dis = lidar_scan
        lidar_points[:, 0] = dis*np.cos(angles)
        lidar_points[:, 1] = dis*np.sin(angles)
        #lidar_points[:, 2] = 1
        ...
        return lidar_points_in_map, lidar_points
